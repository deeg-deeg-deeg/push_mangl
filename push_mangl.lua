...
m = midi.connect(3) -- needs to be the port on which the "Ableton Push 2" is located
mm = midi.connect(3)

local pushyLib = include("lib/pushyINC") -- loading the modified version of the pushy library by @ericmoderbacher
local pushy = {}
pushy.__index = pushy


-- --------------------------------------------------------------------------
-- from here to "init" this is all the untouched code from MANGL by @justmat
-- --------------------------------------------------------------------------
...
function init()
-- ------------------------------------------------
-- here starts the init part from MANGL, untouched
-- ------------------------------------------------
  ...
  -- ------------------------
  -- here begins my init part
  -- ------------------------
  

  color = 120 -- this value sets the color of the pads, change to what you like best!
  notePressed = {}
  ...
  ccButtState = {}          -- create Matrix for all cc Buttons
      for i=1,119 do
        ccButtState[i] = 0     
      end
  
  pitchState = {}          -- create Matrix for pitch settings
      for i=1,7 do
        pitchState[i] = 0     
      end
  ...
  -- setting up the starting LCD Screeen
  -- display consists of 4 rows with 4 lines - each have a length of 17 chars
  -- code for text ist formatted as follows:
  -- pushyLib.text.new(A,"some text",B,C,D)
  -- A sets the row (1,18,35 or 52 (1+17*row))
  -- B sets the line (1,2,3 or 4 of that row)
  -- C is always 17 (length of text) and D you can ignore

  texts[1] = pushyLib.text.new(1,"Sz 0"..szeneIndex.." | RSet 0"..reglerIndex, 1, 17, 1)
  texts[2] = pushyLib.text.new(1,"                 ", 2, 17, 1)
  texts[3] = pushyLib.text.new(1,"                 ", 3, 17, 1)
  texts[4] = pushyLib.text.new(1,"                 ", 4, 17, 1)
  
  texts[5] = pushyLib.text.new(18,"                 ", 1, 17, 1)
  texts[6] = pushyLib.text.new(18,"                 ", 2, 17, 1)
  texts[7] = pushyLib.text.new(18,"                 ", 3, 17, 1)
  texts[8] = pushyLib.text.new(18,"                 ", 4, 17, 1)
  
  texts[9] = pushyLib.text.new(35,"                 ", 1, 17, 1)
  texts[10] = pushyLib.text.new(35,"                 ", 2, 17, 1)
  texts[11] = pushyLib.text.new(35,"                 ", 3, 17, 1)
  texts[12] = pushyLib.text.new(35,"                 ", 4, 17, 1)
  
  texts[13] = pushyLib.text.new(52,"                 ", 1, 17, 1)
  texts[14] = pushyLib.text.new(52,"                 ", 2, 17, 1)
  texts[15] = pushyLib.text.new(52,"                 ", 3, 17, 1)
  texts[16] = pushyLib.text.new(52,"          by deeg", 4, 17, 1)
  
  updateRegMx() -- calling this to send the text definied above to the LCD Screen of the push
  
  killAllButtons() -- making sure all buttons are off
  killAllButtons() -- calling twice because of a mysterious glitch
  
  for i=36,101 do -- check if a preset is running on MANGL and if this preset has samples already playing, if so light the buttons so that we can see the sample slots that are already on fire
    killButtonStart(i)
  end
...
-- ------------------------------------------------------------------------------------------
-- from here until noted down below this is the part where all the push interaction is setup 
-- ------------------------------------------------------------------------------------------
...
function checkPlay()
  ...
  for i=92,98 do
      k = params:get(i-91 .. "play")
      if k == 2 then lightButtonPreset(i)
        else
      end
  end
end
...
m.event = function(data) -- event routine taken from ericmoderbacher, listening to incoming midi
  ...
  if mess.type == "note_on" and mess.note >=10 then  
    
    x = mess.note-36

    if notePressed[mess.note-36] == 0 then -- check if sample is aalreaady playing or not, 0 means: no
      lightButton(mess.note)
      if mess.note >= 92 and mess.note <= 98 then -- these are the buttons 92-98 which start and stop sample 1-7
 
        whaaaaat=mess.note-91
    ...
    elseif notePressed[mess.note-36] == 1 then
      
      killButton(mess.note)
      if mess.note >= 92 and mess.note <= 98 then
  ...
  elseif mess.type == "note_off" and mess.note >=10 and buttonSw == 1 then  -- turn off button if already on
    
    killButton(mess.note)
  
  elseif mess.type == "note_on" and mess.note <=9 then -- touching the top rotary knobs will trigger this
  
    -- texts[4] = pushyLib.text.new(1,"Touch Regler", 4, 17, 1)
      
  elseif mess.type == "pitchbend" and ccButtState[43] == 0 and ccButtState[42] == 0 then
    
    if pbOnOff == 0 then
      pitchState[reglerIndex]=params:get(reglerIndex .. "pitch")
      pbOnOff = 1
    end
    
    pp = pbCalc(mess.val)
    
    params:set(reglerIndex .. "pitch", pp)
    updateRegMx()
  
  
  elseif mess.type == "pitchbend" and ccButtState[43] == 1 and ccButtState[42] == 0 then -- if button43 is on pitchbend controls position value
    
    pp = pbCalc(mess.val)/24
    
    params:set(reglerIndex .. "pos", pp)
    updateRegMx()
    
    
  elseif mess.type == "pitchbend" and ccButtState[42] == 1 and ccButtState[43] == 0 then -- if button42 is on pitchbend controls speed value
    
    pp = pbCalc(mess.val)/24*300
    if mess.val <= 8000 then pp=pp*-1 end
    params:set(reglerIndex .. "speed", pp)
    updateRegMx()
  
    
  elseif mess.type == "note_off" and mess.note == 12 then
    pbOnOff = 0
    params:set(reglerIndex .. "pitch", pitchState[reglerIndex])
    updateRegMx()

  elseif mess.type == "cc" and mess.cc == 9 and mess.val == 127 then -- button 9 switches Pads Light mode to go off when pad is released instead of on when pushed and off when pushed again - not used at the moment
  ...
  elseif mess.type == "cc" then -- if the message is a cc message go to the checkCC() funktion 
...
function pbCalc(ms) -- calculation of the pitchbend value to fit -24 to 24 pitch, calculation of outgoing values is exponential so values in/decrease slowly

    pp = ((ms-7800)*0.0028)/4.67
    pp = pp*pp
    if mess.val <= 7800 and mess.type == "pitchbend" then pp=pp*-1 end
    print(mess.val)
    if pp >= 24 then
      pp=24
    elseif pp <=-23.9 then
...
function get_sample_name_deeg() -- a variation of original mangl code by justmat to get filename
  long_name = string.match(params:get(reglerIndex .. "sample"), "[^/]*$")
  short_name = string.match(long_name, "(.+)%..+$")
  if string.len(short_name) >= 17 then
    return string.sub(short_name, 1, 10) .. '...' .. string.sub(short_name, -4)
  else
    return short_name
  end
end
...
function updateRegMx() -- update the LCD Screen routine 
  
          texts[5] = pushyLib.text.new(18,"Gain:   "..
          math.floor(params:get(reglerIndex .. "gain") * 10) / 10,
          1, 17, 1) 
        
          texts[6] = pushyLib.text.new(18,"Pos:    "..
          params:get(reglerIndex .. "pos"),
          2, 17, 1) 
       
          texts[7] = pushyLib.text.new(18,"Speed:  "..
          math.floor(params:get(reglerIndex .. "speed") * 10) / 10,
          3, 17, 1) 
        
          texts[8] = pushyLib.text.new(18,"Jitter: "..
          math.floor(params:get(reglerIndex .. "jitter") * 10) / 10,
          4, 17, 1)         
       

          texts[9] = pushyLib.text.new(35,"Size:    "..
          math.floor(params:get(reglerIndex .. "size") * 10) / 10,
          1, 17, 1) 
        
          texts[10] = pushyLib.text.new(35,"Density: "..
          math.floor(params:get(reglerIndex .. "density") * 10) / 10,
          2, 17, 1) 
       
          texts[11] = pushyLib.text.new(35,"DMod:    "..
          params:get(reglerIndex .. "density_mod_amt"),
          3, 17, 1) 
        
          texts[12] = pushyLib.text.new(35,"Pitch:   "..
          params:get(reglerIndex .. "pitch"),
          4, 17, 1)            
        
          texts[13] = pushyLib.text.new(52,"Spread:   "..
          params:get(reglerIndex .. "spread"),
          1, 17, 1)         
        
          texts[14] = pushyLib.text.new(52,""..
          get_sample_name_deeg(),
          2, 17, 1)   
       
end

function updateRegMxB() --  since there are more parameters than can fit on the LCD there is a second routine for updating the alternate param group on LCD screen
  
          texts[5] = pushyLib.text.new(18,"CutOff: "..
          math.floor(params:get(reglerIndex .. "cutoff") * 10) / 10,
          1, 17, 1) 
        
          texts[6] = pushyLib.text.new(18,"Q:      "..
          params:get(reglerIndex .. "q"),
          2, 17, 1) 
       
          texts[7] = pushyLib.text.new(18,"SendD:  "..
          params:get(reglerIndex .. "send"),
          3, 17, 1) 
        
          texts[8] = pushyLib.text.new(18,"Fade:   "..
          math.floor(params:get(reglerIndex .. "fade") * 10) / 10,
          4, 17, 1)         
       

          texts[9] = pushyLib.text.new(35," ",
          1, 17, 1) 
        
          texts[10] = pushyLib.text.new(35," ",
          2, 17, 1) 
       
          texts[11] = pushyLib.text.new(35," ",
          3, 17, 1) 
        
          texts[12] = pushyLib.text.new(35," ",
          4, 17, 1)  
        
          texts[13] = pushyLib.text.new(52," ",
          1, 17, 1)  
...
function checkCC(x)
  ...
  if mess.cc >= 71 and mess.cc <= 79 then -- checking if the rotary knobs are sending values
    
    i = mess.cc - 70
    
  if reglerSwitch == 0 then -- reglerSwitch are Buttons 102-109 which define which sample controls of MANGL we are modifying
      
        if mess.val == 127 then
          
          if i == 1 then
           params:delta(reglerIndex .. "gain", -1)
          elseif i == 2 then
            params:delta(reglerIndex .. "pos", -1)
          elseif i == 3 then
            params:delta(reglerIndex .. "speed", -1)
          elseif i == 4 then
            params:delta(reglerIndex .. "jitter", -1)
          elseif i == 5 then  
            params:delta(reglerIndex .. "size", -1)
          elseif i == 6 then  
            params:delta(reglerIndex .. "density", -1)
          elseif i == 7 then  
            params:delta(reglerIndex .. "density_mod_amt", -1)
          elseif i == 8 then  
            params:delta(reglerIndex .. "pitch", -1)
          elseif i == 9 then  
            params:delta(reglerIndex .. "spread", -1)
          end
    ...
    elseif reglerSwitch == 1 then -- if Button 119 is on this value is 1 and therefore we are editing the secondary control values of the sample
      
        if mess.val == 127 then
        
           if i == 1 then
           params:delta(reglerIndex .. "cutoff", -1)
          elseif i == 2 then
            params:delta(reglerIndex .. "q", -1)
          elseif i == 3 then
            params:delta(reglerIndex .. "send", -1)
          elseif i == 4 then
            params:delta(reglerIndex .. "fade", -1)
          end
  ...
  elseif mess.cc == 14 or mess.cc == 15 then -- check knobs 14,15 .. nothing is assigned to knob 14 atm

    if mess.val == 127 and mess.cc == 15 then  
      print(mess.val)
      params:delta(reglerIndex .. "speed", -0.1)
      
    elseif mess.val == 1 and mess.cc == 15 then
      params:delta(reglerIndex .. "speed", 0.1)
    end
    
    if reglerSwitch == 0 then updateRegMx() end
    redraw()

  
  elseif mess.cc >= 20 and mess.cc <= 27 then -- Buttons 20 to 27 - they were supposed to act a scene selectors but I didn't need them - could be used to load presets??? maybe an idea for the future
    szeneIndex = mess.cc -19
    texts[1] = pushyLib.text.new(1,"Sz 0"..szeneIndex.." | RSet 0"..reglerIndex, 1, 17, 1)
    lightScene(szeneIndex+19)
  
  elseif mess.cc >= 102 and mess.cc <= 109 and mess.val == 127 then --Buttons 102 to 109 set which sample slot of MANGL we are editing

    reglerIndex = mess.cc - 101
    texts[1] = pushyLib.text.new(1,"Sz 0"..szeneIndex.." | RSet 0"..reglerIndex, 1, 17, 1)
    lightRegler(reglerIndex+101)
    
    if reglerSwitch == 1 then
      updateRegMxB()
      -- texts[2] = pushyLib.text.new(1,"Regler B ", 2, 17, 1) -- for dev only
    
    elseif reglerSwitch == 0 then
      updateRegMx()  
      -- texts[2] = pushyLib.text.new(1,"Regler A ", 2, 17, 1) -- for dev only
    end
  
  
  elseif (mess.cc >= 36 and mess.cc <= 63 and mess.val == 127) or   -- AUX System Buttons Check
         (mess.cc >= 85 and mess.cc <= 90 and mess.val == 127) or
         (mess.cc >= 110 and mess.cc <= 119 and mess.val == 127)
         then -- AUX 1-5
    
    x = mess.cc
    
    if ccButtState[x] == 0 then
      print("sollte an sein: "..x)
      ccButtState[x] = 1
      lightButton(mess.cc)
      
    elseif ccButtState[x] == 1 then
      print("sollte aus sein: "..x)
      ccButtState[x] = 0
      killButton(mess.cc)
    
    end
    
-- here listeners for buttons can be added to add more functionality    
...
-- here are the functions for the buttons above


function ccButtAction62() -- has no function atm

end 

function ccButtAction110() -- sets speed to 1
  params:set(reglerIndex .. "speed", 1)
  updateRegMx()
end 

function ccButtAction111() -- sets position to a random value
  rnd = math.random(0, 100)
  params:set(reglerIndex .. "pos", rnd/100)
  updateRegMx()
end 

function ccButtAction112() -- sets pitch to 0
  params:set(reglerIndex .. "pitch", 0)
  updateRegMx()
end
 
function ccButtAction113() -- inverts the speed value
  sp = params:get(reglerIndex .. "speed")
  sp = sp*-1
  params:set(reglerIndex .. "speed", sp)
  updateRegMx()
end 
  
function ccButtAction114() -- sets position to 0
  params:set(reglerIndex .. "pos", 0)
  updateRegMx()
end

function ccButtAction115() -- sets position to 1
  params:set(reglerIndex .. "pos", 1)
  updateRegMx()
end

function ccButtAction119() -- switches between value group 1 and 2 on the LCD Screen of the push
  
  if reglerSwitch == 0 then
      reglerSwitch = 1
      
      updateRegMxB()
      
    elseif reglerSwitch == 1 then
      reglerSwitch = 0
...
function ccButtAction87() -- kills all button lights except those of the pads
...
function clearLEDScreen() 
  
  for v=0,3 do
    for i=1,4 do
      texts[i+(v*4)] = pushyLib.text.new(1+(17*v),"", i, 17, 1)  
      print(i+(v*4).."  "..1+(17*v).."  "..i)
    end
  end
end


function lightButton(x) -- routine to light buttons

 coma = "0x"..convertToHexString(color, 2)
 count = "0x"..convertToHexString(x,2)
 
 if x == mess.note then
  mm:send{0x90} 
  notePressed[x-36] = 1
 
 elseif x == mess.cc then
   mm:send{0xB1}  
 end
...
function lightButtonPreset(x) -- routine used on start to set buttons on if a preset has samples already playing
...
function killAllButtons() -- routine to turn off all pad buttons light
  
  for i=36,99 do
    coma = "0x"..convertToHexString(color, 2)
    count = "0x"..convertToHexString(i,2)
    mm:send{0x90}   --note on
    mm:send{count}   --target cc:99
    mm:send{0x00}
    notePressed[i-36] = 0
  end
...
function killSceneLights() -- routine to turn off all scene buttons light

  for i=20,27 do
    coma = "0x"..convertToHexString(color, 2)
    count = "0x"..convertToHexString(i,2)
    mm:send{0xB1}   --note on
    mm:send{count}   --target cc:99
    mm:send{0x00}
...
function killReglerLights() -- routine to turn off all buttons 102-109 light
...
function lightScene(u)
  
  killSceneLights()
    
  coma = "0x"..convertToHexString(4, 2)
  count = "0x"..convertToHexString(u,2)
...
function lightRegler(y)
  
  killReglerLights()
  
  coma = "0x"..convertToHexString(5, 2)
  count = "0x"..convertToHexString(y,2)
...
function convertToHexString(number, digits) -- helper routine which converts String to Hex -- by CmP
...
-- -------------------------------------------------------------- 
-- and from here on out is the rest of the MANGL code, untouched
-- --------------------------------------------------------------
